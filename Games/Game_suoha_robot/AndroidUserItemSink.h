...
private:
	...
	SUOHA::CGameLogic::handinfo_t all_[GAME_PLAYER], show_[GAME_PLAYER], cover_[GAME_PLAYER];
	...
	//过牌/梭哈/跟注/加注/弃牌/看牌
	typedef int prob_t[OP_MAX][Win_Lost_Max][Stock_Sataus_Max];
	//排除非使能操作权重后的随机概率
	typedef STD::Weight weight_t[Win_Lost_Max][Stock_Sataus_Max];
	static std::map<int, weight_t> weights_;
	static STD::Weight weightLookCard_[Win_Lost_Max][Stock_Sataus_Max];
private:
