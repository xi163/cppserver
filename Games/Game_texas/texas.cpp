...
namespace TEXAS {
	...
	//同花顺之间比较大小
 	static bool As123scbyPointG(std::vector<uint8_t> const* a, std::vector<uint8_t> const* b) {
		return CGameLogic::CompareCards(&a->front(), a->size(), &b->front(), b->size()/*, true*/, Ty123sc) > 0;
 	}
	//同花顺之间比较大小
	...
	//顺子之间比较大小
	static bool As123byPointG(std::vector<uint8_t> const* a, std::vector<uint8_t> const* b) {
		return CGameLogic::CompareCards(&a->front(), a->size(), &b->front(), b->size()/*, true*/, Ty123) > 0;
	}
	//顺子之间比较大小
	static bool As123byPointL(std::vector<uint8_t> const* a, std::vector<uint8_t> const* b) {
		return CGameLogic::CompareCards(&a->front(), a->size(), &b->front(), b->size()/*, true*/, Ty123) < 0;
	}
	//顺子之间比较大小
	static void SortCardsByPoint_src123(std::vector<uint8_t> const** psrc, int n, bool ascend) {
		if (ascend) {
			std::sort(psrc, psrc + n, As123byPointL);
		}
		else {
			std::sort(psrc, psrc + n, As123byPointG);
		}
	}

	//同花之间比较大小
	static bool AsscbyPointG(std::vector<uint8_t> const* a, std::vector<uint8_t> const* b) {
		return CGameLogic::CompareCards(&a->front(), a->size(), &b->front(), b->size()/*, true*/, Tysc) > 0;
	}
	//同花之间比较大小
	static bool AsscbyPointL(std::vector<uint8_t> const* a, std::vector<uint8_t> const* b) {
		return CGameLogic::CompareCards(&a->front(), a->size(), &b->front(), b->size()/*, true*/, Tysc) < 0;
	}
	//同花之间比较大小
	static void SortCardsByPoint_srcsc(std::vector<uint8_t> const** psrc, int n, bool ascend) {
		if (ascend) {
			std::sort(psrc, psrc + n, AsscbyPointL);
		}
		else {
			std::sort(psrc, psrc + n, AsscbyPointG);
		}
	}
	...
	//dst0 std::vector<std::vector<uint8_t>>& 存放所有同花色n张连续牌-同花顺
	//dst1 std::vector<std::vector<uint8_t>>& 存放所有非同花n张连续牌-顺子
	//dstc std::vector<std::vector<uint8_t>>& 存放所有同花n张非连续牌-同花
	void CGameLogic::ClassifyCards(uint8_t const* src, int len, int n,
		...
		EnumSameColorCards(pdst, c, cpy, cpylen, n, ctx, dstc_);
		{
			...
			//printf("\n--- *** 所有同花色%d张连续牌-同花顺\n", n);
			//for (std::vector<std::vector<uint8_t>>::const_iterator it = dst0.begin();
			//	it != dst0.end(); ++it) {
			//	PrintCardList(&it->front(), it->size());
			//}
		}
		{
			...
			//psrc组与组之间按牌值降序排列(从大到小)
			SortCardsByPoint_src123(psrc, c, false);
			for (int i = 0; i < c; ++i) {
				assert(psrc[i]->size() == n);
				std::vector<uint8_t> v(&psrc[i]->front(), &psrc[i]->front() + psrc[i]->size());
				dst1.push_back(v);
			...
			//printf("\n--- *** 所有非同花%d张连续牌-顺子\n", n);
			//for (std::vector<std::vector<uint8_t>>::const_iterator it = dst1.begin();
			//	it != dst1.end(); ++it) {
			//	PrintCardList(&it->front(), it->size());
			//}
		}
		{
			...
			//psrc组与组之间按牌值降序排列(从大到小)
			SortCardsByPoint_srcsc(psrc, c, false);
			for (int i = 0; i < c; ++i) {
				assert(psrc[i]->size() == n);
				std::vector<uint8_t> v(&psrc[i]->front(), &psrc[i]->front() + psrc[i]->size());
				dstc.push_back(v);
			...
			//printf("\n--- *** 所有同花%d张非连续牌-同花\n", n);
			//for (std::vector<std::vector<uint8_t>>::const_iterator it = dstc.begin();
			//	it != dstc.end(); ++it) {
			//	PrintCardList(&it->front(), it->size());
			//}
		}
...
// 				}
	...
	//返回枚举牌后的剩余牌
	//src uint8_t const* 一副手牌 + 桌面牌
	//dst CardData const& 枚举牌
	//cpy uint8_t *cpy 剩余牌 cpylen int& 余牌数量
	void CGameLogic::GetLeftCards(
		uint8_t const* src, int len,
		EnumTree::CardData const& dst,
		uint8_t* cpy, int& cpylen) {
		assert(len >= dst.size());
		cpylen = 0;
		//遍历一副手牌查找当前牌
	...
	HandTy CGameLogic::AnalyseCards(uint8_t const* src, int len, handinfo_t& hand) {
		//printf("--- *** 待分析手牌如下\n");
		//PrintCardList(src, len);
		assert(len > 0);
		uint8_t psrc[len];// = { 0 };
		...
		if (enumList.v123sc.size() > 0 && std::min(len, 5) == 5) {
			//查看所有枚举牌型
			...
			std::copy(card.begin(), card.end(), std::back_inserter(hand.cards_));
			GetLeftCards(src, len, hand.cards_, cpy, cpylen);
			if (/*classify.*/cpylen > 0) {
				SortCards(/*classify.*/cpy, /*classify.*/cpylen, false, false, false);
				for (int i = 0; i < std::min(/*classify.*/cpylen, 1); ++i) {
					hand.cards_.emplace_back(/*classify.*/cpy[i]);
				}
				if (len == MAX_TOTAL && /*classify.*/cpylen > 0) {
					sp_cards.resize(1);
					memcpy(&sp_cards.front(), &/*classify.*/cpy[0], 1);
				}
			}
		}
		...
		else if (enumList.vsc.size() > 0 && std::min(len, 5) == 5) {
			hand.ty_ = Tysc;//同花
			EnumTree::CardData& card = enumList.vsc[0];
			std::copy(card.begin(), card.end(), std::back_inserter(hand.cards_));
		}
		else if (enumList.v123.size() > 0 && std::min(len, 5) == 5) {
			//查看所有枚举牌型
			//enumList.PrintEnumCards(false, TyAll);
			hand.ty_ = Ty123;//顺子
			...
			if (/*classify.*/cpylen > 0) {
				SortCards(/*classify.*/cpy, /*classify.*/cpylen, false, false, false);
				for (int i = 0; i < std::min(/*classify.*/cpylen, 2); ++i) {
					hand.cards_.emplace_back(/*classify.*/cpy[i]);
				}
				if (len == MAX_TOTAL && /*classify.*/cpylen >= 2) {
					sp_cards.resize(2);
					memcpy(&sp_cards.front(), &/*classify.*/cpy[0], 2);
				}
			}
		}
			...
			if (/*classify.*/cpylen > 0) {
				SortCards(/*classify.*/cpy, /*classify.*/cpylen, false, false, false);
				for (int i = 0; i < std::min(/*classify.*/cpylen, 3); ++i) {
					hand.cards_.emplace_back(/*classify.*/cpy[i]);
				}
				if (len == MAX_TOTAL && /*classify.*/cpylen >= 3) {
					sp_cards.resize(3);
					memcpy(&sp_cards.front(), &/*classify.*/cpy[0], 3);
				}
			}
		}
		else {
			...
			hand.cards_.resize(std::min(classify.cpylen, 5));
			memcpy(&hand.cards_.front(), &classify.cpy[0], std::min(classify.cpylen, 5));
			if (len == MAX_TOTAL && classify.cpylen >= 5) {
				sp_cards.resize(5);
				memcpy(&sp_cards.front(), &classify.cpy[0], 5);
			}
		}
		if (len == MAX_TOTAL) {
			if (hand.cards_.size() == 5) {
				for (std::vector<uint8_t>::const_iterator it = hand.cards_.begin();
					it != hand.cards_.end(); ++it) {
					for (int i = 0; i < MAX_COUNT; ++i) {
						if (src[i] == *it) {
							hand.vec[0].emplace_back(i);//手牌
						}
					}
					for (int i = 0; i < len - MAX_COUNT; ++i) {
						if (src[i + MAX_COUNT] == *it) {
							hand.vec[1].emplace_back(i);//桌面牌
						}
					}
				}
			}
			else if (hand.ty_ == Tysp) {
				for (std::vector<uint8_t>::const_iterator it = sp_cards.begin();
					it != sp_cards.end(); ++it) {
					for (int i = 0; i < MAX_COUNT; ++i) {
						if (src[i] == *it) {
							hand.vec[0].emplace_back(i);//手牌
				...
				assert((hand.cards_.size() + sp_cards.size()) == 5);
				for (std::vector<uint8_t>::const_iterator it = hand.cards_.begin();
					it != hand.cards_.end(); ++it) {
					for (int i = 0; i < MAX_COUNT; ++i) {
						if (src[i] == *it) {
	...
	int CGameLogic::CompareCardPointBy_123(
		uint8_t const* src, int srcLen,
		uint8_t const* dst, int dstLen, bool clr) {
		uint8_t psrc[MaxSZ] = { 0 }, pdst[MaxSZ] = { 0 };
		assert(srcLen > 0);
		...
		SortCards(psrc, srcLen, false, true, true);
		SortCards(pdst, dstLen, false, true, true);
		if (GetCardValue(psrc[srcLen - 1]) == A &&
			GetCardValue(psrc[srcLen - 2]) != K) {
			SortCards(psrc, srcLen, true, true, true);
		}
		if (GetCardValue(pdst[dstLen - 1]) == A &&
			GetCardValue(pdst[dstLen - 2]) != K) {
		...
		while (i >= 0 && j >= 0) {
			uint8_t p0 = CGameLogic::GetCardPoint(psrc[i--]);
			if (i == -1) {
				p0 = CGameLogic::GetCardValue(psrc[i + 1]);
			}
			uint8_t p1 = CGameLogic::GetCardPoint(pdst[j--]);
			if (j == -1) {
				p1 = CGameLogic::GetCardValue(pdst[j + 1]);
			}
			if (p0 != p1) {
				return p0 - p1;
			}
	...
	int CGameLogic::CompareCards(
		...
		//同花顺之间比较
		case Ty123scRoyal:
		case Ty123sc: return CompareCardPointBy_123(src, srcLen, dst, dstLen, false);
		//同花之间比较
		case Tysc: return CompareCardPointBy(src, srcLen, dst, dstLen, false);
		//顺子之间比较
		case Ty123: return CompareCardPointBy_123(src, srcLen, dst, dstLen, false);
		//散牌之间比较
		case Tysp: return CompareCardPointBy(src, srcLen, dst, dstLen, false);
		//铁支之间比较
		case Ty40: {
			assert(srcLen > 0);
			assert(dstLen > 0);
		...
		case Ty30: {
			...
			if (cpylen_0 && cpylen_1) {
				//三张大小相同，比较散牌大小
				return CompareCardPointBy(cpy_0, cpylen_0, cpy_1, cpylen_1, false);
			}
			break;
		}
		//两对之间比较 ♦A ♣A ♣8 ♣K ♠8
    ...
    void CGameLogic::TestEnumCards(char const* filename)
    {
        ...
        if (flag > 0)
        {
			...
			//手牌牌型分析
			CGameLogic::AnalyseCards(cards, n, hand);
			CGameLogic::PrintCardList(&hand.cards_.front(), hand.cards_.size());
			//查看所有枚举牌型
			//hand.rootEnumList->PrintEnumCards(false, TyAll);
			//查看手牌特殊牌型
			//hand.PrintSpecCards();
