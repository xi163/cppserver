...
#define MAX_CARD_TOTAL	TEXAS::MaxCardTotal	//牌总个数
#define GAME_PLAYER		TEXAS::MaxPlayer	//最多9人局
#define MIN_GAME_PLAYER TEXAS::MinPlayer	//至少3人局
#define MAX_COUNT		TEXAS::MaxCount		//每人2张牌
#define TBL_CARDS		TEXAS::TblCards		//桌面5张牌
#define MAX_ROUND       TEXAS::MaxRound		//最大局数
#define MAX_TOTAL		(MAX_COUNT+TBL_CARDS)

//德州扑克
namespace TEXAS {
	const int MaxCardTotal = 52;	//牌总个数，除去大小王，52张牌
	const int MaxPlayer = 9;		//最多9人局
	const int MinPlayer = 2;		//至少3人局
	const int MaxCount = 2;			//每人2张牌
	const int TblCards = 5;			//桌面5张牌
	...
	enum HandTy {
		...
		Ty123         = 4,	//顺子：花色不同的连续五张牌(A2345最小，10JQKA最大)
		Tysc          = 5,	//同花：花色相同的五张牌，非顺子
		Ty32          = 6,	//葫芦：一组三条加上一组对子
		Ty40          = 7,	//四条(铁支)：除了四张值相同的牌外没有其它牌型
		Ty123sc       = 8,	//同花顺：花色相同的连续五张牌(A2345最小，10JQKA最大)
	...
	public:
		...
		public:
			...
			handinfo_t(handinfo_t const& ref) {
				...
				std::copy(ref.vec[0].begin(), ref.vec[0].end(), std::back_inserter(vec[0]));
				std::copy(ref.vec[1].begin(), ref.vec[1].end(), std::back_inserter(vec[1]));
			}
	...
	public:
		...
		static HandTy AnalyseCards(uint8_t const* src, int len, handinfo_t& hand);
		//牌比大小
	...
	public:
		...
		static int CompareCardPointBy_123(
			uint8_t const* src, int srcLen,
			uint8_t const* dst, int dstLen, bool clr);
	private:
		//按照尾墩5张/中墩5张/头墩3张依次抽取枚举普通牌型
