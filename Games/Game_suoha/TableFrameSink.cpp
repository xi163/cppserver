...
bool CGameTable::OnUserEnter(int64_t userId, bool lookon) {
	...
	if (table_->GetGameStatus() < GAME_STATUS_READY) {
		...
		OnEventGameScene(player->GetChairId(), false);
	}
	//机器人中途加入
...
// 					table_->GetTableId(), StringStat(table_->GetGameStatus()).c_str(), maxAndroid_, sliceMatchSeconds_, totalMatchSeconds_, table_->GetRealPlayerCount(), table_->GetRobotPlayerCount(), table_->GetPlayerCount());
			...
			if (writeRealLog_) {
				//满足游戏人数，开始游戏
				LOG_INFO("tableID[%d][%s]补充机器人(max=%d)时间(dt=%.1f|%.1f)，满足游戏人数(real=%d AI=%d total=%d)，开始游戏!",
					table_->GetTableId(), StringStat(table_->GetGameStatus()).c_str(), maxAndroid_, sliceMatchSeconds_, totalMatchSeconds_, table_->GetRealPlayerCount(), table_->GetRobotPlayerCount(), table_->GetPlayerCount());
			}
			GameTimerReadyOver();
		}
...
void CGameTable::OnGameStart() {
	...
	for (int i = 0; i < GAME_PLAYER; ++i) {
		...
		//player->set_viplevel(player->GetVip());
		player->set_headid(player->GetHeaderId());
		//player->set_headboxindex(player->GetHeadboxId());
		//player->set_headimgurl(player->GetHeadImgUrl());
		suoha::HandCards* handcards = player->mutable_handcards();
...
void CGameTable::AnalysePlayerCards() {
	...
	//库存高于上限值，需要放水
		...
		LOG_INFO("StockLowLimit:%ld StockScore:%ld StockHighLimit:%ld 算法 随机", StockLowLimit, StockScore, StockHighLimit);
	}
}
		...
		if (table_->IsRobot((uint32_t)i) > 0) {
			if (writeRealLog_) {
				LOG(INFO) << "\n========================================================================\n"
					...
					<< SUOHA::CGameLogic::StringCards(&(handCards_[i])[0], MAX_COUNT)
					<< "] 牌型 [" << SUOHA::CGameLogic::StringHandTy(SUOHA::HandTy(all_[i].ty_)) << "] takeScore_[" << takeScore_[i] << "]\n";
			}
		}
		else {
			if (writeRealLog_) {
	...
	if (table_->IsRobot(currentWinUser_) > 0) {
		if (writeRealLog_) {
			LOG(INFO) << "\n--- *** [" << strRoundID_ << "] 机器人 [" << currentWinUser_ << "] " << UserIdBy(currentWinUser_) << " 手牌 ["
				<< SUOHA::CGameLogic::StringCards(&(handCards_[currentWinUser_])[0], MAX_COUNT)
				<< "] 牌型 [" << SUOHA::CGameLogic::StringHandTy(SUOHA::HandTy(all_[currentWinUser_].ty_)) << "] 最大牌\n\n\n";
		}
	}
	else {
		if (writeRealLog_) {
...
bool CGameTable::OnUserGiveUp(uint32_t chairId, bool timeout) {
	...
	if (player && !player->IsRobot()) {
		table_->RefreshRechargeScore(player->GetUserId(), chairId);
...
bool CGameTable::OnGameMessage(uint32_t chairId, uint8_t subid, uint8_t const* data, size_t len) {
	...
	case suoha::SUB_C_PASS_SCORE: {	//过牌
		...
		if (bPlaying_[chairId] && !userItem->IsRobot()) {
...
//发送场景
bool CGameTable::OnEventGameScene(uint32_t chairId, bool lookon) {
	//char msg[1024];
	//snprintf(msg, sizeof(msg), " --- *** tableID[%d][%s][%s][%d] chairId[%d]",
	//	table_->GetTableId(), StringStat(table_->GetGameStatus()).c_str(), strRoundID_.c_str(), currentTurn_ + 1, chairId);
	//LOG(INFO) << __FUNCTION__ << msg;
...
//游戏结束
bool CGameTable::OnEventGameConclude(uint32_t chairId, uint8_t flags)
{
	for (int i = 0; i < GAME_PLAYER; ++i) {
...
void CGameTable::OnTimerGameEnd() {
	...
	if (/*table_->GetRealPlayerCount() > 0 && */table_->GetPlayerCount() >= MIN_GAME_PLAYER) {
		...
		table_->SetGameStatus(GAME_STATUS_READY);
		gameStatus_ = GAME_STATUS_READY;
		maxAndroid_ = rand_.betweenInt(1, 4/*table_->GetRoomInfo()->maxAndroidCount*/).randInt_mt();
