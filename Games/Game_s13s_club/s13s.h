...
namespace S13S {
	...
	//至尊青龙>一条龙>十二皇族>三同花顺>三套炸弹>全大>全小>凑一色>       >四套冲三>五对冲三>六对半>三顺子>三同花
	//
	//手牌类型：从小到大
	enum HandTy {
		////// 普通牌型
	...
	public:
		...
		//dundata_t 单墩数据
		struct dundata_t {
			dundata_t() :dt_(DunNil), ty_({ Tysp }), c(0), cards({ 0 }) {
			}
		...
		class groupdun_t {
		public:
			groupdun_t() :specialTy(Tysp), start(DunNil) {
		...
		public:
			handinfo_t()
				:rootEnumList(NULL)/*, specialTy_(Tysp)*/, chairID(-1),
		...
		protected:
			//确定手牌牌型
			void CalcHandCardsType(uint8_t const* src, int len, int roomID, int tableID, int chairID);
		public:
			//判断手动摆牌是否倒水
			//dt DunTy 指定为哪墩
			...
			//返回手牌特殊牌型
			inline HandTy SpecialTy() { return spec_groups.size() > 0 ? spec_groups.front().specialTy : Tysp; }
		public:
			//临时对象析构不释放资源
			bool forswap;
			//玩家座椅ID
	...
	public:
		...
		static int AnalyseHandCards(uint8_t const* src, int len, int n, handinfo_t& hand,
			int roomID = 0, int tableID = 0, int chairID = 0);
		//单墩牌型判断(3/5张牌)
		//dt DunTy 指定为第几墩
