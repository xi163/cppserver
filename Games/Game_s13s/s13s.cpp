...
//#include "../QPAlgorithm/pb2Json.h"

void openLog(int roomID, int tableID, const char *p, ...) {
	time_t tt;
	time(&tt);
	struct tm* tm = localtime(&tt);
	//filename
	char filename[256] = { 0 };
	snprintf(filename, sizeof(filename), "./log/s13s/%04d%02d%02d_%04d_%04d.log", tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, roomID, tableID);
...
//				}
	...
	//同花顺之间/同花之间/顺子之间比较大小
 	static bool As123scbyPointG(std::vector<uint8_t> const* a, std::vector<uint8_t> const* b) {
		return CGameLogic::CompareCards(&a->front(), &b->front(), a->size(), true, Tysp) > 0;
 	}
	//同花顺之间/同花之间/顺子之间比较大小
...
// 				}
	...
	int CGameLogic::AnalyseHandCards(uint8_t const* src, int len, int n, handinfo_t& hand,
		int roomID, int tableID, int chairID) {
		int c = 0;
		...
		HandTy tyLeaf = Tysp, tyChild = Tysp, tyRoot = Tysp;
...
#endif
		hand.CalcHandCardsType(src, len, roomID, tableID, chairID);
		return c;
	}
	...
	//确定手牌牌型
	void CGameLogic::handinfo_t::CalcHandCardsType(uint8_t const* src, int len, int roomID, int tableID, int chairID) {
...
// 				}
		...
		if (enum_groups_tmp.size() == 0) {
			openLog(roomID, tableID, "FUCK handcard[%d] enum_groups_tmp.size = 0 !!!", chairID);
			openLog(roomID, tableID, "%s", CGameLogic::StringCards(src, len).c_str());
		}
	...
	void CGameLogic::TestEnumCards(int size) {
		...
		while (1) {
			...
			//有特殊牌型时
			//pause = (hand.specialTy_ != S13S::Tysp);
			//有三同花顺/三同花/三顺子时
				...
				for (int i = 0; i < GAME_PLAYER; ++i) {
					if (true) {
						...
						for (int k = 1; k <= 4; ++k) {
							...
							case 2: {
								//第二次从13张手牌中随机抽5张放入尾墩
								int j = 0;//GlobalFunc::RandomInt64(0, MAX_COUNT - 5);
							...
							case 3: {
								...
								//第三次从余下手牌中随机抽5张放入中墩
								int j = 0;//GlobalFunc::RandomInt64(0, rspdata.cpy().size() - 5);
							...
							case 4: {
								...
								//第四次从余下手牌中随机抽3张放入头墩
								int j = 0;//GlobalFunc::RandomInt64(0, rspdata.cpy().size() - 3);
								assert(j == 0);
			...
			}
			{
				...
				for (int i = 0; i < GAME_PLAYER; ++i) {
					if (true) {
						...
						int c = S13S::CGameLogic::AnalyseHandCards(&(handCards[i])[0], MAX_COUNT, enum_group_sz, handInfos[i]);
						//有特殊牌型时
						//if (handInfos[i].specialTy_ == S13S::Tysp) {
						//	goto restart;
						//}
						//查看所有枚举牌型
						//handInfos[i].rootEnumList->PrintEnumCards(false, S13S::Ty123sc);
...
// 						}
						if (rand() % 2 == 0) {
							...
							//直接选择枚举中的一组进行比牌
							int groupindex = 0;//GlobalFunc::RandomInt64(0, handInfos[i].enum_groups.size() - 1);
