...
namespace cipher
{
	...
	int AES_ECB_Cipher::encrypt(const unsigned char* src, uint32_t src_len, unsigned char* dest, uint32_t& len)
	{
		// check input null pointer
		if (NULL == src)
		{
			//printf("AES_ECB_Cipher::encode src is NULL ");
			return -1;
		}
		...
		unsigned char encode_buf[1024/*MAX_DECODE_BUF_LEN*/] = { 0 };
		...
		// PKCS5Padding rules：补(16-len)个(16-len)
		...
		{
			//printf("AES_ECB_Cipher::encode, fail to encrypt src");
			return -1;
		}
		dest[len] = 0;
		return 0;
	...
	int AES_ECB_Cipher::decrypt(const unsigned char* src, uint32_t src_len, unsigned char* dest, uint32_t& len)
	{
		// check input null pointer
		if (NULL == src)
		{
		...
		{
			printf("AES_ECB_Cipher::decode, src len has to be divided by 16");
			return -1;
		}
