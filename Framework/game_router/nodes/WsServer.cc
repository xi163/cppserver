...
bool RouterServ::onCondition(const muduo::net::InetAddress& peerAddr, muduo::net::InetRegion& peerRegion) {
...
void RouterServ::onConnection(const muduo::net::TcpConnectionPtr& conn) {
	...
	if (conn->connected()) {
		...
		EntryPtr entry(new Entry(Entry::TypeE::TcpTy, conn, "客户端", "路由服"));
