...
bool ApiServ::onHttpCondition(const muduo::net::InetAddress& peerAddr, muduo::net::InetRegion& peerRegion) {
...
void ApiServ::onHttpWriteComplete(const muduo::net::TcpConnectionPtr& conn) {
	conn->getLoop()->assertInLoopThread();
#if 0
	//不再发送数据
	conn->shutdown();
...
void ApiServ::processHttpRequest(
	...
	if (req.path() == "/") {
		...
		if (server_state_ == kRepairing) {
			...
			int errcode = Order(req, rsp, conn, buf, receiveTime);
#ifdef _STAT_QPS_
...
// std::string ApiServ::onProcess(std::string const& reqStr, muduo::Timestamp receiveTime, int& errcode, std::string& errmsg, boost::property_tree::ptree& latest, int& testTPS) {
...
//2.游戏启动刷新一次
//3.redis广播通知刷新一次
bool ApiServ::refreshAgentInfo() {
	WRITE_LOCK(agent_info_mutex_);
	agent_info_.clear();
	return mgo::LoadAgentInfos(
		document{}
		<< "agentid" << 1 << "score" << 1
		<< "status" << 1 << "cooperationtype" << 1
		<< "md5code" << 1 << "descode" << 1 << finalize,
		{},
		agent_info_);
}
...
//2.游戏启动刷新一次
//3.redis广播通知刷新一次
void ApiServ::refreshWhiteList() {
	//开启了IP访问白名单功能
	if (whiteListControl_ == eApiCtrl::kOpenAccept) {
		//Accept时候判断，socket底层控制，否则开启异步检查
		RunInLoop(httpserver_.getLoop(), std::bind(&ApiServ::refreshWhiteListInLoop, this));
	}
...
bool ApiServ::refreshWhiteListSync() {
	//Accept时候判断，socket底层控制，否则开启异步检查
...
bool ApiServ::refreshWhiteListInLoop() {
	//Accept时候判断，socket底层控制，否则开启异步检查
	assert(whiteListControl_ == eApiCtrl::kOpenAccept);
	httpserver_.getLoop()->assertInLoopThread();
	white_list_.clear();
...
//请求挂维护/恢复服务 status=0挂维护 status=1恢复服务
bool ApiServ::repairServer(containTy servTy, std::string const& servname, std::string const& name, int status, std::string& rspdata) {
...
bool ApiServ::repairServer(std::string const& queryStr, std::string& rspdata) {
	std::string errmsg;
	containTy servTy;
	std::string name;
	int status;
...
void ApiServ::repairServerNotify(std::string const& msg, std::string& rspdata) {
	std::string errmsg;
	containTy servTy;
	std::string name;
	int status;
