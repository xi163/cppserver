...
bool ApiServ::onCondition(const muduo::net::InetAddress& peerAddr, muduo::net::InetRegion& peerRegion) {
...
void ApiServ::onConnection(const muduo::net::TcpConnectionPtr& conn) {
	...
	if (conn->connected()) {
		...
		muduo::net::websocket::hook(
			std::bind(&ApiServ::onVerify, this,
				std::placeholders::_1),
...
bool ApiServ::onVerify(muduo::net::http::IRequest const* request) {
	switch (verify_) {
	case true: {
