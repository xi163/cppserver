...
int main(int argc, char* argv[]) {
	...
	std::string logdir = pt.get<std::string>("game_api.logdir", "./log/game_api/");
	std::string logname = pt.get<std::string>("game_api.logname", "game_api");
	int loglevel = pt.get<int>("game_api.loglevel", 1);
	...
	std::string ip = pt.get<std::string>("game_api.ip", "");
	int16_t port = pt.get<int>("game_api.port", 9888);
	int16_t httpPort = pt.get<int>("game_api.httpPort", 9788);
	int16_t numThreads = pt.get<int>("game_api.numThreads", 10);
	int16_t numWorkerThreads = pt.get<int>("game_api.numWorkerThreads", 10);
	int kMaxQueueSize = pt.get<int>("game_api.kMaxQueueSize", 1000);
	int kMaxConnections = pt.get<int>("game_api.kMaxConnections", 15000);
	int kTimeoutSeconds = pt.get<int>("game_api.kTimeoutSeconds", 3);
	//管理员挂维护/恢复服务
	std::string strAdminList = pt.get<std::string>("game_api.adminList", "192.168.2.93,");
	//证书路径
	std::string cert_path = pt.get<std::string>("game_api.cert_path", "");
	//证书私钥
	std::string private_key = pt.get<std::string>("game_api.private_key", "");
