...
public:
	...
	virtual inline bool Valid() { return ASSERT_NOTNULL_REF(baseInfo_)->userId != INVALID_USER/* && tableId_ != INVALID_TABLE && chairId_ != INVALID_CHAIR*/; }
	...
	virtual inline int64_t  GetUserId() { return ASSERT_NOTNULL_REF(baseInfo_)->userId; }
	virtual inline const std::string GetAccount() { return ASSERT_NOTNULL_REF(baseInfo_)->account; }
	virtual inline const std::string GetNickName() { return ASSERT_NOTNULL_REF(baseInfo_)->nickName; }
	virtual inline uint8_t  GetHeaderId() { return ASSERT_NOTNULL_REF(baseInfo_)->headId; }
	...
	virtual inline int64_t GetUserScore() { return ASSERT_NOTNULL_REF(baseInfo_)->userScore; }
	virtual inline void SetUserScore(int64_t userScore) { ASSERT_NOTNULL_REF(baseInfo_)->userScore = userScore; }
	...
	virtual inline const uint32_t GetIp() { return ASSERT_NOTNULL_REF(baseInfo_)->ip; }
	virtual inline const std::string GetLocation() { return ASSERT_NOTNULL_REF(baseInfo_)->location; }
	...
	virtual inline int64_t GetTakeMaxScore() { return ASSERT_NOTNULL_REF(baseInfo_)->takeMaxScore; }
	virtual inline int64_t GetTakeMinScore() { return ASSERT_NOTNULL_REF(baseInfo_)->takeMinScore; }
