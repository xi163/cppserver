...
public:
	...
	virtual bool CanJoinTable(std::shared_ptr<CPlayer> const& player, int64_t clubId, uint16_t excludeId = INVALID_TABLE);
	virtual void CanJoinTableInLoop(std::shared_ptr<CPlayer> const& player, int64_t clubId, uint16_t excludeId, bool& rc, bool& ok);
