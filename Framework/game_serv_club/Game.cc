...
#include "proto/GameClubServer.Message.pb.h"
...
void GameServ::Start(int numThreads, int numWorkerThreads, int maxSize) {
	...
	Warnf("GameServ = %s rpc:%s 房间号[%d] %s numThreads: I/O = %d worker = %d et[%d]", server_.ipPort().c_str(), rpcserver_.ipPort().c_str(), roomId_, getModeMsg(kClub).c_str(), numThreads, numWorkerThreads, et_);
