...
HallServ::HallServ(muduo::net::EventLoop* loop,
	...
	//gateRpcClients_.setConnectionCallback(
	//	std::bind(&HallServ::onConnection, this, std::placeholders::_1));
	//gateRpcClients_.setMessageCallback(
	//	std::bind(&HallServ::onMessage, this,
	//		std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
	rpcClients_[servTyE::kRpcGateTy].clients_ = &gateRpcClients_;
	rpcClients_[servTyE::kRpcGateTy].ty_ = servTyE::kRpcGateTy;
...
void HallServ::onZookeeperConnected() {
	...
	}
	{
		...
		if (ZOK == zkclient_->getClildren(
			...
			names,
			std::bind(
				&HallServ::onGateRpcWatcher, this,
				placeholders::_1, std::placeholders::_2,
				placeholders::_3, std::placeholders::_4,
				placeholders::_5), this)) {
			std::string s;
			...
			rpcClients_[servTyE::kRpcGateTy].add(names);
			rangeGateRpcTest();
		}
	}
	{
...
void HallServ::onGateRpcWatcher(int type, int state,
	const std::shared_ptr<ZookeeperClient>& zkClientPtr,
	const std::string& path, void* context) {
	//网关服RPC ip:port
	std::vector<std::string> names;
	if (ZOK == zkclient_->getClildren(
		...
		_LOG_WARN("可用网关服[RPC]列表%s", s.c_str());
		rpcClients_[servTyE::kRpcGateTy].process(names);
		rangeGateRpcTest();
	}
}

void HallServ::rangeGateRpcTest() {
	xsleep(5000);
	Rpc::ClientConnList clients;
	rpcClients_[servTyE::kRpcGateTy].clients_->getAll(clients);
	for (Rpc::ClientConnList::iterator it = clients.begin();
		it != clients.end(); ++it) {
		Rpc::GameGateService service(*it, 3);
