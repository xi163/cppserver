	...
	namespace server {
		void Service::GetNodeInfo(
			...
			case 0:
				...
				break;
			case 1:
				rsp.set_host(gServer->http_proto_ + gServer->httpserver_.ipPort()/* + gServer->path_http_login_*/);
				rsp.set_domain(gServer->http_proto_ + gServer->httpserver_.ipPort() + gServer->path_http_login_);
			}
