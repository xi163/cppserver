...
namespace utils {
	...
	{
		/*uint32_t*/unsigned int a, b, c, d, e;
		typedef union {
			unsigned char c[64];
			/*uint32_t*/unsigned int l[16];
		} CHAR64LONG16;
#ifdef SHA1HANDSOFF
		...
		/* The following had better never be used because it causes the
			...
			*/
		CHAR64LONG16* block = (char const64LONG16*)buffer;
#endif
	...
	void SHA1Final(SHA1_CTX* context, unsigned char digest[20])
	{
		...
		for (i = 0; i < 2; i++)
		{
			/*uint32_t*/unsigned int t = context->count[i];
			int j;
	...
	char const* SHA1(char const* src, unsigned len, char dst[], int upper) {
		...
		static size_t const size = sizeof(hash);
		SHA1_CTX ctx;
		SHA1Init(&ctx);
		SHA1Update(&ctx, (unsigned char const*)src, len);
		SHA1Final(&ctx, hash);

		int  j = 0;
		for (int i = 0; i < size; i++) {
			upper ?
				sprintf(dst + j, "%02X", hash[i]) :
				sprintf(dst + j, "%02x", hash[i]);
			j += 2;
		}
...
main(int argc, char** argv)
{
	...
	char dst[32 + 1] = { 0 };
	utils::SHA1(src.c_str(), src.length(), dst, 1);
}
